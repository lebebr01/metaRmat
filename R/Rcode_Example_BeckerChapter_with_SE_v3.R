...
#--------------------------------------------------------------------
 OS <- function(x) {
  ...
  nvar <- ncol(x)
  p <- nvar
  pp=p*(p-1)/2 #number of correlations
  covmtx=matrix(0,nrow=pp,ncol=pp)
  x=array(0,dim=c(p,p,p,p))
  y=array(0,dim=c(p,p,p,p))
  covm=array(0,dim=c(p,p,p,p))
   for (s in 1:(p-1)){
   for (t in (s+1):p){
   for (u in s:(p-1)){
   for (v in (u+1):p){
    if (s!=u | t<=v) 
   {x[s,t,u,v]=0.5*mr[s,t]*mr[u,v]*((mr[s,u])^2+(mr[s,v])^2+(mr[t,u])^2+(mr[t,v])^2)+mr[s,u]*mr[t,v]+mr[s,v]*mr[t,u]
   y[s,t,u,v]=mr[s,t]*mr[s,u]*mr[s,v]+mr[t,s]*mr[t,u]*mr[t,v]+mr[u,s]*mr[u,t]*mr[u,v]+mr[v,s]*mr[v,t]*mr[v,u]
   covm[s,t,u,v]=(x[s,t,u,v]-y[s,t,u,v])}
   else x[s,t,u,v]=y[s,t,u,v]=covm[s,t,u,v]=0
  }} }}
  ...
  for (v in (u+1):p){
   if (p>3){
   if (u<(p-1) && s<(p-1)){
   if (((s-1)*(p-s+1)+t-s)<((u-1)*(p-1)+v-u)){
  covmt[((s-1)*(p-1)+t-s),((u-1)*(p-1)+v-u)]=covmt[((u-1)*(p-1)+v-u),((s-1)*(p-1)+t-s)]=covm[s,t,u,v]}
   if (((s-1)*(p-1)+t-s)==((u-1)*(p-1)+v-u))
   {covmt[((s-1)*(p-1)+t-s),((u-1)*(p-1)+v-u)]=covm[s,t,u,v]}
   }
   if (u==(p-1) && s<(p-1)){
   covmt[((s-1)*(p-1)+t-s),(2*p-3+v-u)]=covmt[(2*p-3+v-u),((s-1)*(p-1)+t-s)]=covm[s,t,u,v]}
   if (u==(p-1) && s==(p-1)){
   covmt[(2*p-3+t-s),(2*p-3+v-u)]=covm[s,t,u,v]}
  }
   if (p==3){
   if (((s-1)*(p-s+1)+t-s)<((u-1)*(p-1)+v-u)){
   covmt[((s-1)*(p-1)+t-s),((u-1)*(p-1)+v-u)]=covmt[((u-1)*(p-1)+v-u),((s-1)*(p-1)+t-s)]=covm[s,t,u,v]}
